...
from asciimatics.widgets import Frame, ListBox, Layout, Divider, VerticalDivider, Label, \
    MultiColumnListBox, Button, Widget
...
class HSDMainView(Frame):
    def __init__(self, screen, hsd_info):
        ...
        # Layout TUI Flags ----------------------------------------------------------------------------------------------
        layout_tui_flags = Layout([100])
        self.add_layout(layout_tui_flags)
        layout_tui_flags.add_widget(Label("- Output folder: {}".format(self._hsd_info_model.tui_flags.output_folder)))

        if self._hsd_info_model.tui_flags.acq_name is not None and self._hsd_info_model.tui_flags.acq_name != "":
            layout_tui_flags.add_widget(Label("- Acquisition name: {}".format(self._hsd_info_model.tui_flags.acq_name)))
        else:
            layout_tui_flags.add_widget(Label("- Acquisition description: STWIN_Acq"))

        if self._hsd_info_model.tui_flags.acq_desc is not None and self._hsd_info_model.tui_flags.acq_desc != "":
            layout_tui_flags.add_widget(Label("- Acquisition description: {}".format(self._hsd_info_model.tui_flags.acq_desc)))
        else:
            layout_tui_flags.add_widget(Label("- Acquisition description: "))

        if self._hsd_info_model.tui_flags.file_config != '' and os.path.exists(self._hsd_info_model.tui_flags.file_config):
            layout_tui_flags.add_widget(Label("- Device configuration file: {}".format(self._hsd_info_model.tui_flags.file_config)))
        else:
        ...
        if self._hsd_info_model.tui_flags.time_sec != -1:
            ...
            layout_tui_flags.add_widget(Label("- Duration: Endless log mode"))
        layout_tui_flags.add_widget(Divider())
...
class HSDLoggingView(Frame):
    ...
    def __init__(self, screen, hsd_info):
        ...
        if self._hsd_info_model.tui_flags.file_config is not None:
            if self._hsd_info_model.tui_flags.file_config != '' and os.path.exists(self._hsd_info_model.tui_flags.file_config):
                self.device_config_label = Label("- Device configuration file: {}".format(self._hsd_info_model.tui_flags.file_config))
                # layout_tui_flags.add_widget(Label("- Device configuration file: {}".format(self._hsd_info_model.tui_flags.file_config)))
            else:
        ...
        if self._hsd_info_model.tui_flags.time_sec == -1:
            ...
            self.remaining_time_lbl = Label(" Remaining time: {}".format(self._hsd_info_model.tui_flags.time_sec))
        ...
        if self._hsd_info_model.tui_flags.interactive_mode:
            ...
            if self._hsd_info_model.is_log_manually_stopped is not True:
                if not self._hsd_info_model.is_log_started:
                    ...
                    try:
                        ...
                        self._hsd_info_model.tui_flags.file_config = None
                ...
                if self._hsd_info_model.is_log_started:
                    ...
                    if self._hsd_info_model.tui_flags.time_sec != -1:
                        ...
                        remaining_time = self._hsd_info_model.tui_flags.time_sec - delta_time.seconds
