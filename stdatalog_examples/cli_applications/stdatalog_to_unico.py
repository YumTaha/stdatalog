...
"""
This script, `stdatalog_to_unico.py`, is designed to convert data from acquisition folders
...
from stdatalog_core.HSD_utils.exceptions import MissingDeviceModelError, MissingISPUOutputDescriptorException, MissingTagsException
...
def show_help(ctx, param, value):
    ...
    if value and not ctx.resilient_parsing:
        ...
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -s all", fg='cyan')
        # Extract data for all active sensors and save it in CSV format in the specified output folder.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -o Output_Folder_Path -f CSV -s all", fg='cyan')
        # Extract data for a specific sensor named SENSOR_NAME and save it in the default output format.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -s SENSOR_NAME", fg='cyan')
        ...
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -st 3 -et 6 -wt -s SENSOR_NAME", fg='cyan')
        # Extract data for all active sensors and include annotations in the exported data.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -t -s all", fg='cyan')
        ...
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -cdm 255 255 custom_model.json -s SENSOR_NAME", fg='cyan')
        # Specify the size of each data chunk to be 1000 samples for the sensor SENSOR_NAME.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -cs 1000 -s SENSOR_NAME", fg='cyan')
        # Extract data for all active sensors, aggregate the data into a single file, and save it in CSV format.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -ag single_file -f CSV -s all", fg='cyan')
        # Extract data for all active sensors, aggregate the data into a single file, and save it in TXT format.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -ag single_file -f TXT -s all", fg='cyan')
        # Extract raw data (not multiplied by sensitivity) for the sensor SENSOR_NAME and save it in CSV format.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -r -f CSV -s SENSOR_NAME", fg='cyan')
        # Extract data for all active sensors, split the data per tags, and include untagged data sections in the output files.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -ag split_per_tags -wu -s all", fg='cyan')
        # Extract data for all active sensors using the 'mlc_tool' naming convention for column names in the exported files.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -cl mlc_tool -s all", fg='cyan')
        ...
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -ag split_per_tags -wu -cl mlc_tool -s all", fg='cyan')
        # Run the script in debug mode to check for corrupted data and timestamps.
        click.secho("   python stdatalog_to_unico.py Acquisition_Folder_Path -d -s SENSOR_NAME", fg='cyan')
